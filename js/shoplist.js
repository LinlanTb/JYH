$(function(){
	...
	$.get("json/goods.json",function(data){
		for(let x=0;x<12;x++){
				let oLi = "<li><a href='goodDetail.html'><img src='"+data[x].img+"'><b >"+data[x].describe+"</b><p><i>￥"+data[x].price+"</i>￥"+data[x].oldpri+"</p><font>月销"+data[x].msell+"件</font></a></li>";
				$("#goodshow").append(oLi);
	...
	$.get("json/goods.json",function(data){
		...
		for(let i=0;i<pag;i++){
			...
			$("#page a").on("click",function(){
				...
				let oLi = "<li><a href='goodDetail.html'><img src='"+data[i].img+"'><b >"+data[i].describe+"</b><p><i>￥"+data[i].price+"</i>￥"+data[i].oldpri+"</p><font>月销"+data[i].msell+"件</font></a></li>";
